...
export function MessageBubble({ message, threadA, threadB }: Props) {
  ...
  if (isRelayed) {
    ...
    const direction = isA ? `${threadA.label} \u2192 ${threadB.label}` : `${threadB.label} \u2192 ${threadA.label}`;
    return (
  ...
  return (
    <div
      className={`animate-slide-in flex flex-col gap-1 py-2 ${
